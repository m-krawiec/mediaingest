...
from photo_ingest.errors import ConflictError, IngestError, SourceError
...
from photo_ingest.scan import ScanResult, SourceFile, scan_source, write_tree_snapshot
...
def run(args: IngestArgs, cfg: Config) -> IngestRun:
    """Execute a full ingest run.  Returns the IngestRun state (for testing/logging).
    ...
    # Set up event emitter and logging handler
        ...
        with mounted_readonly(args.source_device, cfg.ingest, log) as mountpoint:
            ...
            if copy_mode == "dcim":
                ...
                if not source_root.is_dir():
                    raise SourceError(
                        f"No DCIM directory found on {args.source_device}. "
            ...
            if args.dry_run:
                log.info("[dry-run] Would copy %d files (%.1f MiB) from %s",
            ...
            rsync_copy(
                source_root,
                nvme_dest / ("DCIM" if copy_mode == "dcim" else "_card"),
            ...
            # 10. Compute checksums on NVMe copy
            nvme_copy_root = nvme_dest / ("DCIM" if copy_mode == "dcim" else "_card")
            ...
            hdd_copy_root = hdd_dest / ("DCIM" if copy_mode == "dcim" else "_card")
            vresult = verify(
                scan_result.files,
                nvme_copy_root,
                hdd_copy_root,
...
def _generate_import_id(
    ...
    while True:
        ...
        if not nvme.exists() and not hdd.exists():
            return import_id
        if not auto_seq:
            # Conflict will be caught by _check_conflict
            return import_id
        seq += 1
        if seq > 999:
...
def _sync_ingest_dir_to_hdd(nvme_ingest_dir: Path, hdd_ingest_dir: Path) -> None:
    """Copy updated _ingest/ artifacts to HDD."""
    import shutil
    hdd_ingest_dir.mkdir(parents=True, exist_ok=True)
